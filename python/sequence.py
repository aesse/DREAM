...
class PWM(object):
    ...
    def getRC(self, swap = [('A', 'T'), ('C', 'G')] ):
        ...
        for s in swap:
            ...
            new_bg = self.bg.swapxcopy(s[0],s[1])
        return PWM(new_fg, new_bg)
    ...
    def maxscore(self, sequence):
        """ Find matches to the motif in a specified sequence.
            ...
            (maxscore, maxindex, valid) 
            To be valid, sequence must contain only letters in columns
            with probability > 0"""
        maxscore = None
        maxindex = None
        validity = None
        for i in range(len(sequence)-self.length+1):
            ...
            if maxscore == None:
                ...
                maxindex = i
                validity = valid
        return (maxscore, maxindex, validity)
